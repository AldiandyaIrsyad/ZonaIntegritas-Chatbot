...
import structlog
from app.core.interfaces.infra import ChunkVector, IDocumentParser, IVectorStore
...
class IngestionService(IIngestionService):
    ...
    async def ingest_document(self, doc_id: str) -> None:
        ...
        try:
            ...
            logger.info(
                "rag.ingestion.parsing_complete",
                doc_id=doc_id,
                stage="chunking",
            ...
            logger.info(
                "rag.ingestion.chunking_complete",
                doc_id=doc_id,
                stage="embedding",
                parent_chunks=len(parent_chunk_data),
            ...
            logger.info(
                "rag.ingestion.embedding_complete",
                doc_id=doc_id,
                stage="vector_upsert",
            ...
            try:
                ...
                logger.error(
                    "rag.ingestion.status_update_failed",
                    error=type(rollback_err).__name__,
                    exc_info=True,
                )
            raise

    async def _get_document(self, doc_id: str) -> Any:
        ...
        from app.knowledge_base.model import PDFDocument  # type: ignore
