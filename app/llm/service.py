...
from app.core.interfaces.infra import ILLMConnection
...
class LLMService:
    ...
    Args:
        connection (ILLMConnection): The low-level infrastructure connection.
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning("Model encoding not found, falling back to cl100k_base", model=self.model)
            self.encoder = tiktoken.get_encoding("cl100k_base")
        logger.info("LLMService initialized", model=self.model, max_tokens=self.max_tokens)
    ...
    async def stream_response(
        ...
        logger.debug("Starting LLM stream", model=self.model, num_messages=len(context_payload))
