...
from app.ram.service import MAX_PREMISE_CONTEXTS
...
async def nli_streaming_generate(
    ...
    context_embs = None
    if contexts and getattr(ram_service, "enabled", True):
        embedding_provider = getattr(ram_service, "embedding_provider", None)
        if embedding_provider:
            top_contexts = contexts[:MAX_PREMISE_CONTEXTS]
            context_texts = [ctx.text for ctx in top_contexts]
            context_embs = await embedding_provider.embed_texts(context_texts)
