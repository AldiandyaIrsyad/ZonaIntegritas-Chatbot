...
async def chat_stream(
    ...
    Sends a user message (and optional files) to the LLM for a given session and streams back
    ...
    Args:
        ...
        files (List[UploadFile]): Optional uploaded files (images or PDFs).
        service (ChatService): Dependency-injected chat service instance.
    
    Returns:
        StreamingResponse: Text stream of LLM response tokens.
    """
    return await service.process_chat_message(session_id, message, files)
