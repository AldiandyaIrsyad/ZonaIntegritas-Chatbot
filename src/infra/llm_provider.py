            ...
            if isinstance(msg.content, list):
                for block in msg.content:
                    if block.get("type") == "text":
                        content_to_count += block.get("text", "")
                    elif block.get("type") == "image_url":
                        total += 250 # Rough estimate for image tokens
            else:
                content_to_count = msg.content
                
            # Padding accounts for OpenAI's IM_START/IM_END boundary tokens
            total += len(encoder.encode(content_to_count)) + 4 
        return total
    ...
    def _prune_context(self, raw_history: List[Dict[str, str]], max_tokens: int) -> List[Dict[str, str]]:
        ...
        for msg in raw_history:
            ...
            try:
                ...
                if isinstance(parsed_content, list) and len(parsed_content) > 0 and "type" in parsed_content[0]:
                    # Filter out custom UI metadata blocks
                    content = [b for b in parsed_content if b.get("type") in ["text", "image_url"]]
